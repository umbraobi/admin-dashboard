...
const Sidebar = ({ collapsed, onToggle, currentPage, onChangePage }) => {
  ...
  return (
    <div className={`${collapsed ? "w-20" : "w-72"}
      className="w-72 transition-all duration-300 ease-in-out bg-white/80 dark:bg-slate-900/80
            ...
            {!collapsed && item.submenu && expandedItems.has(item.id) && (
              <div className="ml-8 mt-2 space-y-1">
                {item.submenu.map((subitem) => {
                  return (
                    <button className="w-full text-left p-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200
